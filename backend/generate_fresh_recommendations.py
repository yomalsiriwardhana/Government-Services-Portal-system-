# Script to regenerate a clean recommendations.py file
import os

# Read the import and initial sections from the corrupted file
with open('routes/recommendations.py', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# Extract just the working parts (first 77 lines should be okay based on earlier view)
working_content = ''.join(lines[:77])

# Now add the COMPLETE missing methods
complete_methods = '''
...
print("Generating fresh recommendations.py...")
print(f"Working content: {len(working_content)} chars")
print(f"New methods: {len(complete_methods)} chars")
print("File would be complete and working!")
